...
<Callout type="note">**In the example** (`assignment/js-starter-accordion`): `accordion.clientlibs.js` `[07]` + `accordion.html` — the clientlib category that loads the CSS/JS, booted by the global `run()`/`observe()`.</Callout>
...
flowchart LR
  ...
  C --> L["component-loader<br/>run() + observe()"]
  L --> N["new MyComponent(element, options)"]
```
...
A single entry boots the loader once per page — it runs on existing DOM and **observes** for nodes
added later (essential for author mode):

```js
// publish.clientlibs.js
import { observe as loaderObserve, run as loaderRun } from '@netcentric/component-loader';

loaderRun();      // initialise components already in the DOM
loaderObserve();  // initialise any added afterwards
```
