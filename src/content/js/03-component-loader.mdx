...
<Callout type="note">**Read along in real code:** the `examples/js-component` branch of the `code` project has a complete, annotated **Accordion**. Get it with `git fetch && git checkout examples/js-component`, then open `ui.frontend/.../publish/components/accordion/accordion.clientlibs.js` — every `[NN]` comment maps to a lesson. This one is `[03]` (`register` + the `data-nc` hook in `accordion.html`).</Callout>
