...
## Atomic / shared categories

Beyond per-component styles, the project ships **atomic** clientlibs (e.g.
`academy.publish.atomic.colors`, `…atomic.alignment`) that components can pull in alongside their
own — handy for shared utility classes:

```html
<sly data-sly-use.clientlib="${'/libs/granite/sightly/templates/clientlib.html'}"
     data-sly-call="${clientlib.all @ categories='academy.publish.atomic.colors'}"/>
```
